...
export class LeadListComponent implements OnInit {
  ...
  public addLead():void {
    this.form.reset();
    const dialogRef = this.dialog.open(AddLeadComponent, { disableClose: true,
      width: '80%',
  ...
  viewLead(id:number,type:string):void {
    this.form.reset();
    const dialogRef = this.dialog.open(LeadDetailsComponent,{ disableClose: true,
      width: '80%',
      data: {id:id,type:type,form:this.form,statuss:this.statuss,ratings:this.ratings},
    });
  ...
  bulkModify(status:string){
    this.selection.selected.forEach(element => {
      this.apiService.updateLead(element.id,{"leadStatus":status});
    });
    this.loadLeadList();
